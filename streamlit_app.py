...
st.write("""
        Marketers often have trouble predicting in advance where to get the best value for their \
        limited budgets.\n
        Data can help here, and it is easier than you think.
        """
         )
...
st.write("""
        For this exploration, we'll use an example advertising dataset from a terrific book - \
        *An Introduction to Statistical Learning*.\n
        It contains the advertising budgets (in $'000) for a company at \
        different locations and the number of sales made of a product (in thousands of units).\n
        We'll start with a brief exploration before conducting a type of machine learning \
        called regression. We can then make some predictions.
        """
         )

st.write('## Into the analysis!')
...
st.write("""
        ...
        * **TV budget** _increases reliably_ with sales but plateaus at the higher values\n
        * **Radio budget** also increases with sales but _not as strong_ as TV\n
        * **Newspaper budget** shows the _weakest relationship_ with sales\n
...
st.write('')
st.write(f'The Square Root of the Mean Squared Error (rmse) is one metric we can use which helps \
           measure how accurate our prediction is. Here, the rmse was {rmse}.  \n\
           This means that in general the sales prediction will be out \
